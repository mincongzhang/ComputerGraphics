...
public:
	...
	float* toArray();			//returns point as an array
...
Vector operator-(Point&, Point&);//Vector = Point-Point
Vector operator+(Point&, Point&);//Vector = Point+Point
ostream& operator<<(ostream&,Point);  //writing
istream& operator>>(istream&,Point&); //reading
